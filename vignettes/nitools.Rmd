...
```{r, eval = FALSE}
  ...
  measure = 'thickness';
  region_means = nitools::atlas_agg_group_native(subjects_dir, subjects_list, measure, "lh", atlas, agg_fun = mean);
```
...
You could do the same in standard space with `atlas_agg_group_standard()`, even though it's less common as typically vertex-wise analysis is used in standard space. This requires that you pass the `fwhm` parameter to identify which smoothing value you want:

```{r, eval = FALSE}
  region_means_std = nitools::atlas_agg_group_standard(subjects_dir, subjects_list, measure, "lh", atlas, fwhm = '10', agg_fun = mean);
```
