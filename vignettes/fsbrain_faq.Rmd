...
The default settings used by the the *vis.export.from.coloredmeshes()* function should be fine to reach the 300 DPI typically required for publications, unless you want the image to fill the entire page. In that case, you can further increase the output resolution.
...
You should also have a look at the new `vis.export.from.coloredmeshes` function, which produces an image with colorbar and provides sane defaults for the resolution and suitable text sizes, etc.
...
This required fsbrain version >= 0.4.3 and is implemented in the `vis.export.from.coloredmeshes` function via the `transparency_color` parameter. 
...
* The `transparency_color` can be almost any RGB color, but (1) it should not occur in the brain surface visualization to prevent transparent parts in the brain after post-processing. And (2) because of anti-aliasing, the color may slightly affect the border pixels of the rendered brain surfaces, so a neutral color like 'white', 'black' or some gray value like '#EFEFEF' is maybe more suitable than 'hot pink' or 'magenta'.

I prefer to use white ('#FFFFFF') as the transparency color, therefore I reset the NA color to a very bright gray ('#FEFEFE' in this example): 
...
Resetting the `col.na` is needed/recommended in this case because it is 'white' by default.
