...
concatenates the standard space data for all subjects into a single long
...
- fwhm_per_measure, :
  ...
  vector of strings. The fwhm settings to use, for each measure. If this
  is a string instead of a vector of strings, the same fwhm will be used
  for all measures.
...
values for one measure, concatenated for all subjects. The column names
are a concatination of the measure, "\_fwhm", and the fwhm for that
measure. WARNING: This dataframe can get large if you have many
subjects.
