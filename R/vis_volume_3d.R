...
volvis.voxels <- function(volume, max_render=25000, render_every=8) {
    ...
    if(length(rendered_voxels) > max_render) {
        ...
        cat(sprintf("Will render %d voxels.\n", length(rendered_voxels)));
    }
    ...
    if(nrow(voxel_crs) > 0) {
        ...
        for(idx in seq(length(rendered_voxels))) {
            ...
            surface_ras[idx,] = (vox2ras_tkr() %*% voxel_crs[row_idx,])[1:3];
        }
        rgl::rgl.spheres(surface_ras, r = 0.5, color = "#0000ff", lit=FALSE);
    } else {
