...
#'
#' @description The input table file must be a segmentation or parcellation table, generated by running the FreeSurfer tools 'aparcstats2table' or 'asegstats2table' for your subjects.
#'
#' @param filepath path to input file, a tab-separated file generated by running 'aparcstats2table' or 'asegstats2table'
#'
#' @param z_threshold numerical, the cutoff value for considering a subject an outlier (in standard deviations).
#'
#' @keywords internal
qc.from.segstats.table <- function(filepath, z_threshold=2.8, verbose=FALSE) {
    ...
    for(subject_idx in seq(length(subjects))) {
        ...
        if(num_outlier_regions > 0L) {
            ...
            if(verbose) {
                cat(sprintf("Subject '%s' (# %d) is an outlier in %d regions: %s.\n", subjects[subject_idx], subject_idx, num_outlier_regions, paste(outlier_regions, collapse=', ')));
            } else {
                cat(sprintf("Subject '%s' (# %d) is an outlier in %d regions.\n", subjects[subject_idx], subject_idx, num_outlier_regions));
            }
