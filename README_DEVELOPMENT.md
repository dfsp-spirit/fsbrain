...
- Update the Docker image by creating `docker/fsbrain<version>/`, copying the `Dockerfile` from the most recent image in there, adapting it for the new version (raise R version, freesurferformats version, maybe other deps). Then build and upload the image to Dockerhub (e.g., `docker build -t dfspspirit/fsbrain:0.7.0 .`, then test it, then push it and also update the latest tag (`docker push dfspspirit/fsbrain:0.7.0`, `docker tag dfspspirit/fsbrain:0.7.0 dfspspirit/fsbrain:latest`,
`docker push dfspspirit/fsbrain:latest`). See readme files in `docker/` for details.